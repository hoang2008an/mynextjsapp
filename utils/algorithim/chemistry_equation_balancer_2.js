...
function balance(formulaStr) {
  ...
  try {
    ...
    if (typeof e == "string") {
      ...
      return setMessage("Syntax error: " + e);
    } else if ("start" in e) {
...
function solve(matrix) {
  ...
  if (i == matrix.rowCount() - 1) throw "All-zero solution"; // Unique solution with all coefficients zero
...
function checkAnswer(eqn, coefs) {
  ...
  for (var i = 0; i < elems.length; i++) {
    ...
    for (var k = 0, lhs = eqn.getLeftSide(); k < lhs.length; j++, k++)
      sum = checkedAdd(
        sum,
        checkedMultiply(lhs[k].countElement(elems[i]), coefs[j])
      );
    for (var k = 0, rhs = eqn.getRightSide(); k < rhs.length; j++, k++)
      sum = checkedAdd(
        sum,
        checkedMultiply(rhs[k].countElement(elems[i]), -coefs[j])
      );
    if (sum != 0) throw "Assertion error: Incorrect balance";
  }
}
...
class Equation {
  constructor(lhs, rhs) {
    ...
    this.getLeftSide = function () {
      return cloneArray(lhs);
    };
    this.getRightSide = function () {
      return cloneArray(rhs);
    };
    ...
    this.getElements = function () {
      var result = new Set();
      for (var i = 0; i < lhs.length; i++) lhs[i].getElements(result);
      for (var i = 0; i < rhs.length; i++) rhs[i].getElements(result);
      ...
      function termsToHtml(terms) {
        ...
        for (var i = 0; i < terms.length; i++, j++) {
          ...
          if (coef != 0) {
            ...
            if (coef != 1)
              node.appendChild(
                createSpan(coef.toString().replace(/-/, MINUS), "coefficient")
              );
...
class Term {
  constructor(items, charge) {
    ...
    this.getElements = function (resultSet) {
      resultSet.add("e");
      for (var i = 0; i < items.length; i++) items[i].getElements(resultSet);
    };
  ...
  constructor(items, count) {
    ...
    this.countElement = function (name) {
      ...
      for (var i = 0; i < items.length; i++)
        sum = checkedAdd(
          sum,
          checkedMultiply(items[i].countElement(name), count)
        );
      return sum;
    };
...
function parseTerm(tok) {
  ...
  var elems = new Set();
  for (var i = 0; i < items.length; i++) items[i].getElements(elems);
  ...
  if (items.length == 0) {
    ...
    // Otherwise, a term must not contain an element that starts with lowercase
    for (var i = 0; i < elems.length; i++) {
      if (/^[a-z]+$/.test(elems[i]))
        throw {
...
class Matrix {
  constructor(rows, cols) {
    ...
    for (var i = 0; i < rows; i++) cells.push(cloneArray(row));
    row = null;
    ...
    this.gaussJordanEliminate = function () {
      // Simplify all rows
      for (var i = 0; i < rows; i++) cells[i] = simplifyRow(cells[i]);
