...
class ChemicalEquation {
  constructor(reactants, products) {
    if (reactants instanceof ChemicalExpression) {
      ...
      throw new Error(
        "The reactant must be an instance of ChemicalExpression or a correctly formatted string."
      );
    ...
    for (let i = 0; i < 2; i++) {
      ...
      for (let indexOfMolecule in this[indexOfSide].molecules) {
        ...
        if (indexOfSide == "reactants") {
          ...
          addedAsciiValue =
            Number(this.reactants.molecules.length) + Number(indexOfMolecule);
        }
        this[indexOfSide].molecules[indexOfMolecule].id = String.fromCharCode(
          97 + Number(addedAsciiValue)
        );
  ...
  balance() {
    ...
    subEquations.forEach((subEquation, i) => {
      console.log();
      console.log(
        String(subEquation.substance + ": ").yellow +
          subEquation.standardEquation.toString().green
      );
      console.log(
        String(subEquation.substance + ": ").yellow +
          subEquation.isolatedEquation.toString().green
      );
      console.log(
        String(subEquation.substance + ": ").yellow +
          subEquation.isolatedExpression.toString().green
      );
      ...
      this.idArray.forEach((id) => {
        if (!subEquation.isolatedExpression._hasVariable(id)) {
          ...
          row.push(
            _getCoefficientForVariableInAlgebraicalExpression(
              subEquation.isolatedExpression,
              id
            )
          );
        }
    ...
    equationWithQuantity.reactants = this.reactants.molecules.map(
      (reactant, i) => {
    ...
    equationWithQuantity.products = this.products.molecules.map(
      (product, i) => {
        return {
    ...
    return {
      reactants: this.reactants.map((atom, i) => {
        return {
          atom: atom.atoms
            .map((atom) => {
              if (atom.quantity !== undefined) {
                return atom.identifier + atom.quantity;
              }
      ...
      products: this.products.map((atom, i) => {
        return {
          atom: atom.atoms
            .map((atom) => {
              if (atom.quantity !== undefined) {
  ...
  getAlgebraicalEquationForSubstance(substance) {
    return new algebra.Equation(
      this.reactants._getAlgebraicExpressionForSubstance(substance),
      this.products._getAlgebraicExpressionForSubstance(substance)
    );
  }
  ...
  toLaTex() {
    return (
      this.reactants.toLaTex() + " \\longrightarrow " + this.products.toLaTex()
    );
  }
}
...
class ChemicalExpression {
  constructor(stringOrArrayOfMolecules) {
    if (
      stringOrArrayOfMolecules.constructor === Array &&
      stringOrArrayOfMolecules[0] instanceof Molecule
    ) {
      this.molecules = stringOrArrayOfMolecules;
    } else if (typeof stringOrArrayOfMolecules == "string") {
      stringOrArrayOfMolecules = stringOrArrayOfMolecules
        .replace(/\s/g, "")
        .split("+");
      let molecules = [];
      ...
      throw new Error(
        "The Expression constructor input must be an array whose values are instances of Molecule or a correctly formatted String"
      );
  }
      ...
      molecule.atoms.forEach((atom) => {
        if (atom.identifier == substance) {
          expression = expression.add(
            new algebra.Expression(molecule.id).multiply(
              atom.quantity ? atom.quantity : 1
            )
          );
        }
      });
...
class Molecule {
  constructor(stringOrArrayOfAtoms) {
    if (stringOrArrayOfAtoms.constructor === Array) {
      ...
      throw new Error(
        "The Molecule contructor input must be a String or a correctly formatted Array"
      );
  }
  ...
  toString() {
    ...
    this.atoms.forEach((atom) => {
      string += String(
        atom.identifier + (atom.quantity !== undefined ? atom.quantity : "")
      );
    });
    return string;
  }
  ...
  toLaTex() {
    ...
    this.atoms.forEach((atom) => {
      string += String(
        atom.identifier +
          (atom.quantity !== undefined ? "_" + atom.quantity : "")
      );
    });
    return string;
  }
...
const _checkArrayForNonInteger = (array) => {
  return (
    array.findIndex((fraction) => {
      return !Number.isInteger(fraction.valueOf());
    }) > -1
  );
};

const _getCoefficientForVariableInAlgebraicalExpression = (
  expression,
  variable
) => {
  let number;
...
let a = new ChemicalEquation("FeS + H2SO4 ", "Fe2(SO4)3 + SO2 + H2O");
let answer = a.balance();
console.log("start of answer");
console.log(answer);
//export { ChemicalEquation, ChemicalExpression, Molecule };
