  ...
  return (
    ...
    >
        ...
        {param.map((value, key) => (
          <div key={key + key} className={"inline-block"}>
            <div key={key} className=" flex-col flex text-center mt-20  w-40 ">
              <label className="text-center" key={key}>
                {value}
              </label>
              <input
                placeholder={value}
                className="first:border-l-2 last:border-r-2 border-x-black px-10 flex-row  border-x hover:border-b hover:border-b-pink-600"
                {...register(value)}
              />
            </div>
          </div>
      ...
      <div className="flex justify-center">
        <input
          type="submit"
          className="grid place-items-center bg-pink-600 text-white px-10 py-2 rounded-lg hover:bg-pink-700 p-6 m-4"
          value={"Giải"}
        />
        <input
          type={"reset"}
